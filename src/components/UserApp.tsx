...
export default function UserApp({
  ...
  const handleBuyOfferConfirm = (e: React.FormEvent) => {
    ...
    setTargetNumber('');
    setBuyError('');
    alert('অফার সফলভাবে সাবমিট হয়েছে! এডমিন দ্রুত চালু করে দেবে।');
  };

  // Filter offers based on operator selection, category tab and search query
  const filteredOffers = offers.filter(offer => {
    if (!offer.isActive) return false;
    const matchesOperator = selectedOperator === 'All' || offer.operator === selectedOperator;
    const matchesTab = offer.category === activeTab;
    const matchesSearch = searchQuery === '' || 
      offer.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
      offer.description.toLowerCase().includes(searchQuery.toLowerCase());
    return matchesOperator && matchesTab && matchesSearch;
  });

  const getMethodColor = (method: 'bKash' | 'Nagad' | 'Rocket') => {
    if (method === 'bKash') return 'bg-pink-600';
    if (method === 'Nagad') return 'bg-orange-600';
    return 'bg-blue-600';
  };

  const operatorBgClass = (op: OperatorName) => {
    if (op === 'GP') return 'bg-blue-50';
    if (op === 'Robi') return 'bg-red-50';
    if (op === 'Airtel') return 'bg-rose-50';
    if (op === 'Banglalink') return 'bg-orange-50';
    return 'bg-emerald-50';
  };

  const operatorTextColors: Record<OperatorName, string> = {
    'GP': 'text-blue-600',
    'Robi': 'text-red-600',
    'Airtel': 'text-rose-500',
    'Banglalink': 'text-orange-500',
    'Teletalk': 'text-emerald-600'
  };
  ...
  return (
    <div className="bg-slate-100 flex justify-center py-4 px-2 sm:px-4 min-h-[820px]">
      
      {/* Phone Screen Mockup Frame */}
      <div className="w-full max-w-md bg-white rounded-[32px] shadow-2xl border-8 border-slate-800 overflow-hidden relative flex flex-col min-h-[750px] font-sans">
        
        {/* Device Status/Ear Speaker Notch */}
        <div className="bg-slate-800 h-6 flex justify-between items-center px-6 text-[11px] text-white select-none shrink-0 z-30">
          <span className="font-semibold">6:47 AM</span>
          <div className="w-24 h-4 bg-slate-800 rounded-b-xl mx-auto flex justify-center items-center">
        ...
        {!isLoggedIn ? (
          <div className="flex-1 bg-slate-950 flex flex-col justify-between text-white relative overflow-y-auto pb-6">
            {/* 1. GMS MCD ERP STYLE SYSTEM HEADER */}
            <div className="bg-[#0b192e] border-b border-slate-800 text-center py-4 px-2 select-none">
              <h1 className="text-base font-extrabold tracking-widest text-yellow-400 uppercase font-sans">
                {config.telecomName} ERP SYSTEM
              </h1>
              <p className="text-[9px] text-slate-300 font-bold tracking-widest uppercase mt-0.5 opacity-80">
                ENTERPRISE TELECOM PLANNING
              </p>
            </div>

            {/* 2. MENU & STATS PILLS ROW */}
            <div className="bg-[#0f223d] px-3 py-2 flex items-center justify-between gap-1 border-b border-slate-800 select-none overflow-x-auto shrink-0">
              <button
                type="button"
                onClick={() => alert('Please login first to access the full menu.')}
                className="bg-blue-600 text-white hover:bg-blue-500 px-3 py-1 rounded-md text-[10px] font-bold flex items-center gap-1 transition"
              >
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <span className="bg-purple-900/40 text-purple-400 border border-purple-500/30 px-2 py-0.5 rounded text-[9px] font-bold">
                  Theme
                </span>
                ...
                <span className="bg-emerald-950/60 text-emerald-400 border border-emerald-500/20 px-2 py-0.5 rounded text-[9px] font-mono font-bold">
                  479:54
                </span>
                <span className="bg-blue-950/60 text-blue-400 border border-blue-500/20 px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-wider">
                  TELECOM
                </span>
              </div>
            </div>

            {/* 3. ORANGE RUNNING MARQUEE TICKER */}
            <div className="bg-[#ea580c] text-white px-3 py-1.5 overflow-hidden flex items-center shrink-0 border-b border-orange-700 select-none">
              <span className="text-[9px] font-black bg-white text-orange-600 px-1 rounded mr-2 uppercase shrink-0">ANNOUNCEMENT</span>
              <div className="overflow-x-hidden relative w-full h-4">
                <div className="absolute whitespace-nowrap text-[10px] font-bold animate-marquee">
                  📢 bKash, Nagad, Rocket auto approvals active 24/7. Resellers, get high-commission drive offers immediately. Developed by System Developer Md. Johurul.
                </div>
              </div>
            </div>

            {/* 4. DASHBOARD OVERVIEW TITLE */}
            <div className="px-4 pt-4 select-none">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-black text-white tracking-tight">Dashboard Overview</h2>
                <span className="px-2 py-0.5 bg-blue-600/30 border border-blue-500/40 text-blue-400 rounded text-[9px] font-bold uppercase">
                  ACTIVE
                </span>
              </div>
              <p className="text-[10px] text-slate-400 mt-1 leading-relaxed">
                Role Based Dashboard • Smart Selective Cache Active • Supabase DB Live Sync
              </p>
            </div>

            {/* 5. LIVE UPDATE SIGNAL BOX */}
            <div className="px-4 mt-3 select-none">
              <div className="bg-[#0f223d] border border-slate-800 rounded-2xl p-4 text-center relative overflow-hidden">
                <div className="absolute top-2 right-2 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span>
                  <span className="text-[8px] text-emerald-400 font-bold uppercase tracking-wider">SECURE</span>
                </div>
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">
                  ● LIVE UPDATE SIGNAL
                </p>
                <h3 className="text-2xl font-black text-emerald-400 tracking-wider flex items-center justify-center gap-2 mt-1">
            ...
            {/* 6. BENTO STOCK CARDS */}
            <div className="grid grid-cols-1 gap-2.5 px-4 mt-3 select-none">
              {/* Card 1: Blue - Offers */}
              <div className="bg-blue-600 rounded-2xl p-4 flex items-center justify-between text-white relative overflow-hidden shadow-md">
                <div className="space-y-0.5">
                  <p className="text-[9px] text-blue-200 uppercase font-black tracking-wider">📦 Reseller Drive Packs</p>
                  <h4 className="text-xl font-black">50+ Active</h4>
                </div>
                <div className="text-3xl opacity-85">📱</div>
              </div>

              {/* Card 2: Green - Auto Gateways */}
              <div className="bg-emerald-600 rounded-2xl p-4 flex items-center justify-between text-white relative overflow-hidden shadow-md">
                <div className="space-y-0.5">
                  <p className="text-[9px] text-emerald-200 uppercase font-black tracking-wider">⚡ Automated Gateways</p>
                  <h4 className="text-xl font-black">bKash / Nagad / Rocket</h4>
                </div>
                <div className="text-3xl opacity-85">💸</div>
              </div>

              {/* Card 3: Orange - Support */}
              <div className="bg-orange-500 rounded-2xl p-4 flex items-center justify-between text-white relative overflow-hidden shadow-md">
                <div className="space-y-0.5">
                  <p className="text-[9px] text-orange-200 uppercase font-black tracking-wider">📞 Hotline Helpline</p>
                  <h4 className="text-xl font-black">24/7 Live Agent</h4>
                </div>
                <div className="text-3xl opacity-85">💬</div>
              </div>
            </div>

            {/* 7. THE LOGIN / REGISTER CARD PANEL */}
            <div className="mx-4 mt-5 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl relative">
              ...
              {authMode === 'login' ? (
                <form onSubmit={handleLoginSubmit} className="space-y-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      value={loginPassword}
                      onChange={(e) => loginPassword !== undefined && setLoginPassword(e.target.value)}
                ...
                <form onSubmit={handleRegisterSubmit} className="space-y-3.5 max-h-[380px] overflow-y-auto pr-1">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="space-y-1">
                      <label className="block text-[10px] font-black uppercase text-slate-400 tracking-wider">Password (কমপক্ষে ৬ ডিজিট) *</label>
                      <input
                        type="password"
                        required
                    ...
                    <div className="space-y-1">
                      <label className="block text-[10px] font-black uppercase text-slate-400 tracking-wider">PIN (৪ ডিজিট) *</label>
                      <input
                        type="password"
                        maxLength={4}
                        required
                  ...
                  <div className="space-y-1">
                    <label className="block text-[10px] font-black uppercase text-slate-400 tracking-wider">Reseller Level</label>
                    <select
                      value={regLevel}
            ...
            <div className="text-center pb-4 border-t border-slate-800/80 pt-4">
              {authMode === 'login' ? (
                <p className="text-xs text-slate-400">
                  নতুন রিসেলার একাউন্ট খুলতে চান?{' '}
                  <button
            ...
            {/* 8. DEVELOPER SYSTEM PROFILE FOOTER */}
            <div className="mt-2 mx-4 bg-[#0b192e] border border-slate-800/80 rounded-2xl p-3 flex items-center gap-3 shadow-md select-none">
              <div className="relative shrink-0">
                ...
                />
                <span className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-[#0b192e] rounded-full animate-pulse"></span>
              </div>
              <div className="flex-1 flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-black text-white">Md. Johurul</h4>
                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">System Developer</p>
                </div>
                <div className="h-6 w-px bg-slate-800 mx-3"></div>
                <div className="text-right">
                  <span className="text-[8px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded font-black tracking-widest uppercase">
                    SYS ADMIN
                  </span>
                </div>
              </div>
